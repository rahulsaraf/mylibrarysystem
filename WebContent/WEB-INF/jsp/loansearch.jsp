	...
	<div class="container">
		...
		<c:choose>
    		...
    		<c:when test="${result == 4}">
    			<div class="alert alert-success fade in">
	        	<a href="#" class="close" data-dismiss="alert">&times;</a>
	        	<strong>Success!</strong> Payment submitted successfully.
    			</div>
    		</c:when>
    		...
    		<c:when test="${result == 6}">
    			<div class="alert alert-danger fade in">
	        	<a href="#" class="close" data-dismiss="alert">&times;</a>
	        	<strong>Error!</strong> Cannot submit payment without check-in the book first.
    			</div>
    		</c:when>
    	</c:choose>
					...
					<tbody>
						<c:forEach items="${bookLoans}" var="bookLoan" varStatus="loan">
							<tr>
								...
								<td id="book_id${loan.index}" hidden=""><c:out
								...
								<c:if test="${bookLoan.fine_amt != 0.0}">
								<td id="fine_amt${loan.index}">$<c:out
										value="${bookLoan.fine_amt}" /></td>
								</c:if>
								<c:if test="${bookLoan.fine_amt == 0.0}">
								<td id="fine_amt${loan.index}">-</td>
								</c:if>
								<c:if test="${bookLoan.fine_amt != 0.0 && bookLoan.paid == false}">
								<td><button id="checkbutton" type="button"
											class="btn btn-warning btn-xs" data-toggle="modal"
											data-target="#subPayment" value="${loan.index}"
											onclick="alertme(${loan.index})">
											<span class="glyphicon glyphicon-usd" aria-hidden="true"></span>
								</button></td>
								</c:if>
								<c:if test="${bookLoan.fine_amt != 0.0 && bookLoan.paid == true}">
								<td>Paid</td>
								</c:if>
									...
									<div class="form-group">
										...
										<div class="col-lg-8">
											<input type="text" class="form-control" id="d_dateIn"
												name="dateIn" placeholder="Enter Check-in Date" required>
										</div>
									</div>
		...
		<!-- Modal -->
		<div class="modal fade" id="subPayment" role="dialog">
			<div class="modal-dialog" role="document">
				...
				<div class="modal-content">
					<div class="modal-header">
						<button type="button" class="close" data-dismiss="modal">&times;</button>
						<h4 class="modal-title">Payment - Fine</h4>
					</div>
					<div class="modal-body">
						<p>Click on Submit to confirm payment for this book loan</p>
						<form class="form-horizontal" name="payment"
							action="payment" method="POST">
							<fieldset>
								<div id="criteria">
									<div class="modal-footer">
									<input type="hidden" class="form-control" id="d_loanId3"
												name="d_loanId1" hidden=true >
